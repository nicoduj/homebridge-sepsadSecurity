...
mySepsadSecurityPlatform.prototype = {
      ...
      if (error) {
        ...
        this.SepsadSecurityAPI.getSecuritySystem(result => {
          if (result) {
            ...
            securityService.controlService.subtype = securitySystemName + ' SecurityService';
            ...
            switchService.controlService.subtype = securitySystemName + ' Switch';
    ...
    this.SepsadSecurityAPI.authenticate(error => {
      if (error) {
        ...
        this.SepsadSecurityAPI.getSecuritySystemState(result => {
          this.log.debug('INFO - securitySystem result : ' + JSON.stringify(result));
          if (characteristic == Characteristic.SecuritySystemCurrentState || characteristic == Characteristic.SecuritySystemTargetState )
          {
            switch(result) {
              case SepsadSecurityConst.DISABLED:
                callback(undefined, Characteristic.SecuritySystemTargetState.DISARM); 
                break;
              case SepsadSecurityConst.PARTIAL:
                callback(undefined, Characteristic.SecuritySystemTargetState.NIGHT_ARM); 
                break;
              case SepsadSecurityConst.ACTIVATED:
                callback(undefined, Characteristic.SecuritySystemTargetState.STAY_ARM); 
                break;                
              default:
                callback(-1); 
            }
  ...
  activate(characteristic,value,callback)
  {
    this.SepsadSecurityAPI.sendCommand(
      value ? SepsadSecurityConst.ACTIVATE_COMMAND : SepsadSecurityConst.STOP_COMMAND,
      characteristic,
      callback
    );
  },
...
*/
  ...
  setSecuritySystemTargetStateCharacteristic: function(
    ...
    this.log.debug('INFO - setSecuritySystemTargetStateCharacteristic - ' + value);
    this.activate(characteristic,value,callback);
  },
    ...
    this.SepsadSecurityAPI.getSecuritySystemState(result => {
      for (let s = 0; s < this.foundAccessories.length; s++) {
        let mySepsadSecuritySystemAccessory = this.foundAccessories[a];
          ...
          for (let s = 0; s < mySepsadSecuritySystemAccessory.services.length; s++) {
